...
class CorrelationRule(object):
    ...
    def __init__(self, name, rule_type, logger, parsers, condition, action, stopRule):
        ...
        # Load available functions and their entry points (siem.functions.xxxx)
        functions_module = sys.modules['siem.functions']
        for func_name in functions_module.__all__:
            if func_name == "__init__":
                continue

            # a "main" _must_ be declared in each functions
            module_name = sys.modules['siem.functions.%s' % func_name]
...
if __name__ == "__main__":
    from siem.core.Logger import Logger
